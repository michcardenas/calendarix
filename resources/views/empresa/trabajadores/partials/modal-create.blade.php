<div id="modalCrearTrabajador" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg border border-[#5a31d71A] w-full max-w-md p-6 relative">
        <h2 class="text-lg font-bold text-[#5a31d7] mb-4">➕ Crear Trabajador</h2>
        <form action="{{ route('empresa.trabajadores.store', $empresa->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" name="especialidades" placeholder="Ej: Cortes, Colorimetría, Barba" class="w-full border border-gray-300 rounded px-3 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-[#5a31d7]">
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-[#3B4269]">Bio</label>
                <textarea name="bio" rows="2" maxlength="500" placeholder="Breve descripción del profesional..." class="w-full border border-gray-300 rounded px-3 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-[#5a31d7]"></textarea>
            </div>
