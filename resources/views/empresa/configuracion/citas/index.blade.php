...
<style>
    ...
    /* ===========================
       Base / Helpers
       =========================== */
    body { background: var(--clx-secondary); color: var(--clx-text); }
    .clx-title { color: var(--clx-dark); }
    .text-muted { color: var(--clx-text-light) !important; }
    ...
    .btn-clx-primary:hover { background: var(--clx-primary-dark); border-color: var(--clx-primary-dark); }
    ...
    .btn-clx-outline:hover { background: var(--clx-primary); color: #fff; }
    ...
    /* Badges por estado */
    .badge-state { border-radius: 999px; padding: .4rem .6rem; font-weight: 600; }
    .badge-pendiente  { background: var(--clx-warning); color: #2d2d2d; }
    .badge-confirmada { background: var(--clx-accent); color: #fff; }
    .badge-cancelada  { background: var(--clx-danger); color: #fff; }
    .badge-completada { background: var(--clx-primary-dark); color: #fff; }

    /* Paginación */
    .pagination .page-link { color: var(--clx-primary); border-color: var(--clx-border); }
    .pagination .page-link:hover { background: var(--clx-secondary); }
    .pagination .active .page-link { background: var(--clx-primary); border-color: var(--clx-primary); }

    /* Modales */
    .clx-modal .modal-content { border: 1px solid var(--clx-border); border-radius: var(--clx-radius); box-shadow: var(--clx-shadow-lg); }
    .clx-modal .modal-header { background: var(--clx-primary); color: #fff; border-bottom: none; }
    .clx-modal .modal-title { color: #fff; }
</style>
...
<div class="container py-4">
    ...
    {{-- Fallback por si no llega $trabajadores desde el controlador --}}
    @php
        $trabajadores = $trabajadores ?? \App\Models\Trabajador::where('negocio_id', $id)
            ->orderBy('nombre')
            ->get(['id','nombre']);
    @endphp
    ...
    <div class="clx-card p-3 mb-3">
        <form method="GET" action="{{ route('empresa.configuracion.citas', $id) }}" class="row g-2">
            ...
            <div class="col-12 col-md-2">
                <select name="trabajador_id" class="form-select clx-select">
        ...
        <table class="table table-hover align-middle clx-table mb-0">
            ...
            @forelse($citas as $cita)
                ...
                <tr>
                    ...
                    {{-- Horario --}}
                    <td>
                        <div><span class="text-muted">Inicio:</span> {{ $hIni ?? '—' }}</div>
                        <div><span class="text-muted">Fin:</span> {{ $hFin ?? '—' }}</div>
                        @if(!is_null($durMin))
                            <div><small class="text-muted">{{ $durMin }} min</small></div>
                        @endif
                    </td>
                    ...
                    <td>
                        ...
                        @if(!is_null($cita->precio_cerrado))
                            <small class="text-muted">
                                Precio: ${{ number_format($cita->precio_cerrado / 100, 0, ',', '.') }}
                            </small>
                        @endif
                    </td>
            ...
            @empty
                <tr><td colspan="8" class="text-center py-5 text-muted">Sin resultados</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
