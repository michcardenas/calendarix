...
<div class="max-w-7xl mx-auto px-6 py-10">
    ...
    {{-- Encabezado --}}
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-[#444478]">✏️ Editar Producto</h2>
        <p class="text-sm text-[#7e79c9]">Actualiza los datos de tu producto y gestiona su visibilidad, precios e inventario.</p>
    </div>
    ...
    <form method="POST" action="{{ route('producto.actualizar', $producto->id) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        {{-- Información General --}}
        <div class="bg-[#f6f5f7] shadow rounded-xl p-6">
            <h3 class="text-lg font-semibold text-[#444478] mb-4">🧾 Información general</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-medium text-[#444478]">Nombre</label>
                    <input type="text" name="nombre" class="mt-1 w-full border-gray-300 rounded" required value="{{ old('nombre', $producto->nombre) }}">
                </div>
                <div>
                    <label class="text-sm font-medium text-[#444478]">Código de barras</label>
                ...
                <div>
                    <label class="text-sm font-medium text-[#444478]">Marca</label>
                    <input type="text" name="marca" class="mt-1 w-full border-gray-300 rounded" value="{{ old('marca', $producto->marca) }}">
                </div>
                <div>
                    <label class="text-sm font-medium text-[#444478]">Unidad de medida</label>
                ...
                <div class="md:col-span-2">
                    <label class="text-sm font-medium text-[#444478]">Descripción larga</label>
        ...
        {{-- Precios --}}
        <div class="bg-[#f6f5f7] shadow rounded-xl p-6">
            <h3 class="text-lg font-semibold text-[#444478] mb-4">💰 Precios</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="text-sm font-medium text-[#444478]">Precio de compra</label>
                ...
                <div>
                    <label class="text-sm font-medium text-[#444478]">Precio de venta</label>
                    <input type="text" name="precio_venta" class="mt-1 w-full border-gray-300 rounded" value="{{ old('precio_venta', $producto->precio_venta) }}">
                </div>
                <div>
                    <label class="text-sm font-medium text-[#444478]">Precio promocional</label>
            ...
            <div class="mt-4">
                <label class="inline-flex items-center gap-2">
                    <input type="checkbox" name="activar_oferta" class="rounded border-gray-300 text-[#444478]"
                        {{ old('activar_oferta', $producto->activar_oferta) ? 'checked' : '' }}>
                    <span class="text-sm text-[#444478]">Activar oferta</span>
                </label>
            </div>
        </div>
        ...
        {{-- Inventario --}}
        <div class="bg-[#f6f5f7] shadow rounded-xl p-6">
            <h3 class="text-lg font-semibold text-[#444478] mb-4">📦 Inventario</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-medium text-[#444478]">Stock</label>
                    <input type="number" name="stock" class="mt-1 w-full border-gray-300 rounded" value="{{ old('stock', $producto->stock) }}">
                </div>
                <div>
                    <label class="text-sm font-medium text-[#444478]">Stock mínimo</label>
            ...
            <div class="mt-4">
                <label class="inline-flex items-center gap-2">
                    <input type="checkbox" name="controla_inventario" class="rounded border-gray-300 text-[#444478]"
                        {{ old('controla_inventario', $producto->controla_inventario) ? 'checked' : '' }}>
                    <span class="text-sm text-[#444478]">Controla inventario</span>
                </label>
            </div>
        </div>
        ...
        {{-- Visibilidad --}}
        <div class="bg-[#f6f5f7] shadow rounded-xl p-6">
            <h3 class="text-lg font-semibold text-[#444478] mb-4">👁️ Visibilidad del producto</h3>
            <div class="space-y-3">
                <label class="inline-flex items-center gap-2">
                    <input type="checkbox" name="estado_publicado" class="rounded border-gray-300 text-[#444478]"
                        {{ old('estado_publicado', $producto->estado_publicado) ? 'checked' : '' }}>
                    <span class="text-sm text-[#444478]">Publicado</span>
                </label>
                <label class="inline-flex items-center gap-2">
                    <input type="checkbox" name="mostrar_en_catalogo" class="rounded border-gray-300 text-[#444478]"
                        {{ old('mostrar_en_catalogo', $producto->mostrar_en_catalogo) ? 'checked' : '' }}>
                    <span class="text-sm text-[#444478]">Mostrar en catálogo</span>
                </label>
            </div>
        </div>
        ...
        @if($producto->imagen)
        <div class="mt-6">
            <label class="text-sm font-medium text-[#444478] block mb-2">📸 Imagen actual:</label>
        ...
        <div>
            <label class="text-sm font-medium text-[#444478]">Nueva imagen (opcional)</label>
            <input type="file" name="imagen" class="mt-1 block w-full text-sm border border-gray-300 rounded">
        </div>

        <div>
            <label class="text-sm font-medium text-[#444478]">Agregar nuevas imágenes</label>
        ...
        <div class="text-right pt-6">
            <a href="{{ route('producto.panel') }}"
                class="inline-block px-5 py-2 text-sm text-[#444478] border border-gray-300 rounded hover:bg-gray-50 transition">Cancelar</a>
            <button type="submit"
                class="inline-block px-5 py-2 ml-3 text-sm font-semibold bg-[#444478] text-white rounded hover:bg-[#444478] transition">
                💾 Guardar cambios
            </button>
        </div>
    </form>
    ...
    <div class="mt-10">
        <h4 class="text-lg font-semibold text-[#444478] mb-4">🖼️ Galería de Imágenes</h4>
