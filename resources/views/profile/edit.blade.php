...
@php
    // Por si no lo envían desde el controlador
    $misEmpresas = $misEmpresas ?? (auth()->user()->negocios ?? collect());
@endphp
<style>
...
<div class="clx-container flex min-h-screen">
    {{-- ===== Aside (insertado) ===== --}}
    <aside class="clx-sidebar w-64 bg-[#0f172a] text-white p-5">
        ...
        <div class="clx-user-info mb-6">
            <div class="clx-user-avatar w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-lg font-bold">
                {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
            </div>
            <div class="clx-user-name mt-2 font-medium">{{ auth()->user()->name }}</div>
            <div class="clx-user-email text-sm text-white/70">{{ auth()->user()->email }}</div>
        </div>

        <nav>
            <ul class="clx-nav space-y-1">
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded bg-white/10">
                        <i class="fas fa-home clx-nav-icon w-4"></i>
                        Dashboard
                    </a>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded hover:bg-white/10">
                        <i class="fas fa-calendar-check clx-nav-icon w-4"></i>
                        Mis Citas
                    </a>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded hover:bg-white/10">
                        <i class="fas fa-store clx-nav-icon w-4"></i>
                        Negocios
                    </a>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded hover:bg-white/10">
                        <i class="fas fa-heart clx-nav-icon w-4"></i>
                        Favoritos
                    </a>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded hover:bg-white/10">
                        <i class="fas fa-history clx-nav-icon w-4"></i>
                        Historial
                    </a>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded hover:bg-white/10">
                        <i class="fas fa-user-cog clx-nav-icon w-4"></i>
                        Mi Perfil
                    </a>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center justify-between px-3 py-2 rounded hover:bg-white/10" data-clx-toggle="empresa-submenu">
                        <span class="flex items-center gap-2">
                            <i class="fas fa-briefcase clx-nav-icon w-4"></i>
                            Mi Empresa
                        </span>
                        <i class="fas fa-chevron-down"></i>
                    </a>

                    <ul id="empresa-submenu" class="clx-submenu mt-1 ml-4 hidden">
                        @forelse ($misEmpresas as $empresa)
                            <li>
                                <a href="{{ route('empresa.dashboard', $empresa->id) }}" class="clx-submenu-link block px-3 py-1.5 rounded hover:bg-white/10">
                                    {{ $empresa->neg_nombre_comercial ?? 'Sin nombre comercial' }}
                                </a>
                            </li>
                        @empty
                            <li><span class="text-sm text-white/60">Sin empresas aún</span></li>
                        @endforelse
                    </ul>
                </li>
                <li class="clx-nav-item">
                    <a href="#" class="clx-nav-link flex items-center gap-2 px-3 py-2 rounded hover:bg-white/10">
                        <i class="fas fa-bell clx-nav-icon w-4"></i>
                        Notificaciones
                    </a>
                </li>
            </ul>
    ...
    {{-- ===== Contenido principal (tu perfil) ===== --}}
    <main class="clx-main flex-1">
...
{{-- 🔹 Script para abrir/cerrar el dropdown de "Mi Empresa" --}}
<script>
document.addEventListener('DOMContentLoaded', function () {
    const toggle = document.querySelector('[data-clx-toggle="empresa-submenu"]');
